...
function gmw_base_url() {
  //return trailingslashit(apply_filters('cfpf_base_url', plugins_url('', __FILE__)));
  return trailingslashit(apply_filters('gmw_base_url', THEME_URI . '/core/google-maps-widget/'));
}
...
class GMW {
   function init() {
      if (is_admin()) {
        ...
        add_filter('plugin_action_links_' . gmw_base_url() . __FILE__,
  ...
  function plugin_meta_links($links, $file) {
    $documentation_link = '<a target="_blank" href="' . gmw_base_url() . __FILE__ . '#" title="View Google Maps Widget documentation">Documentation</a>';
  ...
  function dialogs_markup() {
   ...
   function enqueue_scripts() {
     if (is_active_widget(false, false, 'googlemapswidget', true)) {
       wp_enqueue_style('gmw', gmw_base_url() . 'css/gmw.css', array(), GMW_VER);
       wp_enqueue_script('gmw-fancybox', gmw_base_url() . 'js/jquery.fancybox.pack.js', array('jquery'), GMW_VER, true);
       wp_enqueue_script('gmw', gmw_base_url() . 'js/gmw.js', array('jquery'), GMW_VER, true);
     }
    } // enqueue_scripts
    ...
    function admin_enqueue_scripts() {
      if (self::is_plugin_admin_page()) {
        ...
        wp_enqueue_script('sn-cookie', gmw_base_url() . 'js/jquery.cookie.js', array('jquery'), GMW_VER, true);
        wp_enqueue_script('gmw-admin', gmw_base_url() . 'js/gmw-admin.js', array(), GMW_VER, true);
        wp_enqueue_style('gmw-admin', gmw_base_url(). 'css/gmw-admin.css', array(), GMW_VER);
      } // if
